...
static void ETH_MACDMA_Config(void)
{
  ...
  ETH_DeInit();
	...
	//printf("RCC_AHB1ENR = 0x%08X\r\n", RCC->AHB1ENR);
  //printf("RCC_AHB1LPENR = 0x%08X\r\n", RCC->AHB1LPENR);
  //printf("ETH_DMABMR = 0x%08X\r\n", ETH->DMABMR);
  /* Software reset */
  ETH_SoftwareReset();
...
//  ETH_InitStructure.ETH_AutoNegotiation = ETH_AutoNegotiation_Disable; 
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0x04; /* 优先级必须低于 configMAX_SYSCALL_INTERRUPT_PRIORITY */
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 0x00;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);

/* 使能 ETH DMA 接收中断 */
	ETH_DMAITConfig(ETH_DMA_IT_NIS | ETH_DMA_IT_R | ETH_DMA_IT_RBU, ENABLE);
}
